...
let order={number:'PM-'+Date.now().toString().slice(-6),date:new Date().toLocaleString('es-UY'),customer:data,items:cart,shipping:method,payment:payment,subtotal:getSubtotal(),shippingCost:getShippingCost(),total:getOrderTotal(),status:'Pendiente de pago'};localStorage.setItem(ORDER_KEY,JSON.stringify(order));saveCart([]);localStorage.removeItem(CHECKOUT_KEY);window.location.href='Confirmacion.aspx'}
